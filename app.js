...
async function searchWord(raw){
  ...
  try{
    ...
    makeQuiz();
  }catch(e){setStatus("Không tìm thấy từ này. Hãy thử một từ tiếng Anh khác.");result.classList.add("hidden")}
}
...
$("#randomBtn").onclick=()=>{const words=["achieve","describe","progress","confident","independence","success"];const w=words[Math.floor(Math.random()*words.length)];input.value=w;searchWord(w)};
